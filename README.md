...
- [ ] Explain benefit(s) using `client-side routing`?
  > Answer: Client-side routing is used in single page web apps, when the actual website is never left
- [ ] What does AJAX stand for?
  > Answer: Asynchronous JavaScript and XML
- [ ] What are `controlled components` in React?
  > Answer: Input forms whose value is controlled cand called by react
